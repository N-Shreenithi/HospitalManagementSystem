...
public class SystemImplementation implements SystemInterface {
    ...
    public void admitPatient(Patient patient) throws InvalidPatientInformation, DoubleAdmissions {
        ...
        //if (patientDAO.getPatientById(patient.getPatientId()) != null) {
            //throw new DoubleAdmissions("Patient with this ID already exists.");
        
        	//patient_id, doctor_id, admission_date, room_number, reason_for_admission, discharge_stat
        	
        	//int admissionId, int patientId, int doctorId, Date admissionDate, Date dischargeDate,
            //String roomNumber, String reasonForAdmission, boolean 
        else {
...
//	        	LocalDate admissiondate = LocalDate.parse(admissionDate, formatter);
        ...
        // Register the patient in the system (Add patient to the database)
        admissionDAO.addAdmission(admission);

        // Now handle the patient admission logic based on their type
//        String patientType = patient.getPatientType();  // InPatient or OutPatient
//        Admission admission = new Admission(patient.getPatientId(), new Date(), null); // Admission Date only (assuming dischargeDate will be set later)
//        admissionDAO.addAdmission(admission);
    ...
    public void dischargePatient(int patientId) throws InvalidPatientInformation {
        ...
        // Set discharge date and update discharge status
//        admission.setDischargeDate(new Date());
//        admission.setDischargeStatus(true);
        System.out.println("Enter admissionid: ");
        Scanner sc = new Scanner(System.in);
        int admissionid = sc.nextInt();
